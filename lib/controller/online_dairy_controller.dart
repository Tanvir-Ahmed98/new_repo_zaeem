...
class OnlineDairyController extends GetxController {
  final StorageService _storageService = StorageService();
  final ApiService _apiService = ApiService();
  final LocalDatabase _localDatabase = LocalDatabase();

  String? pComId, pUsername, pStudentId, pDairyQueryDate;
  RxList dairyList = [].obs;
  RxBool dairyLoading = true.obs;
  @override
  void onInit() {
    pStudentId = Get.arguments[0];
    pDairyQueryDate = Get.arguments[1];
    getOnlineDariyList();
    super.onInit();
  }

  Future<void> getOnlineDariyList() async {
    await _storageService.init();
    pComId = _storageService.read('comId').toString();

    try {
      var response = await _apiService.get(
          "/PT_CHILD_DIARY_DAILY_REPORT?P_COM_ID=$pComId&P_DATE=$pDairyQueryDate&P_STD_ID=$pStudentId");
      dairyList.value = response['items'];
      dairyLoading.value = false;
    } catch (error) {
      dairyLoading.value = false;
      log("Error: $error");
    }
  ...
  Future<void> downloadAndSaveImageToGallery(
    ...
    Map<String, dynamic> searchingQuery = {
      "dataTableName": "OnlineDairyReport",
      'id': customId
    };
            ...
            if (value.isGranted) {
              ...
              await getExternalStorageDirectory().then((value) async {
                if (value != null) {
                  ...
                  String filePath =
                      '$folderPath/OnlineDairyReport${DateTime.now().day}0${DateTime.now().month}0${DateTime.now().year}$customId.jpg';
